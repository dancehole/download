...
- 🔐 **账号与权限（角色 + 相册授权）**：账号分 `super`（超级管理员：全部相册 + 共享文件 + OSS 设置 + 用户管理）与 `album`（相册管理员：**只能管理被授权的相册**，看不到其他相册（接口返回 404），共享文件/系统设置/用户管理一律 403）。授权是**多对多**（一个账号可管多个相册、一个相册可有多个管理员），后台「用户管理」勾选相册即可授权 / 取消；相册详情页还有「相册管理员」区块：填一个用户名就能**一键创建管理员并自动授权本相册**（自动生成 12 位强密码，只显示一次可复制）。权限以数据库实时状态为准（停用账号 / 取消授权 / 改角色**下一次请求立即生效**，不用等 token 过期），且不允许删掉或停用最后一个超级管理员
...
| 角色 | 能做什么 | 看不到 / 做不了 |
|------|----------|----------------|
| `super` 超级管理员 | 全部相册（卡片显示归属账号）、共享文件、OSS 设置、用户管理 | — |
| `album` 相册管理员 | 被授权相册的查看/上传/改标签/改设置/重发分享链接/删照片/删除该相册 | 其他相册（接口一律返回 `404 活动不存在`，不泄露存在性）、共享文件、系统设置、用户管理（`403`），且不能新建相册 |

授权是**多对多**：一个账号可被授权多个相册；一个相册可有多个管理员账号。

数据表：
- `photographer.role`（`super` / `album`）、`photographer.is_active`（停用即失效）
- `album_admin_acl(photographer_id, event_id, granted_by)`：一行 = 一条授权；账号/相册删除时级联清理。`init_db()` 启动时自动建表 + `ALTER` 加列，现有 `admin` 自动升级为 `super`

### 设计要点

- **权限以数据库实时状态为准**：JWT 里虽然写了 role，但每次请求都会查库校验账号是否存在、是否启用、是否被授权 —— 所以**停用账号 / 取消授权 / 改角色，下一次请求立即生效**（不用等 72h token 过期），老 token 也不会失效
- **前端只隐藏入口，后端强制校验**：相册管理员后台看不到「用户管理 / 共享文件 / 设置 / 新建」入口，即使绕过前端直接调接口也是 403/404
- **安全兜底**：不允许删除或停用最后一个可用的超级管理员；不能删除/停用/降级当前登录的账号；账号名下还有相册或共享文件时拒绝删除（提示改用「停用」，保留历史 `created_by` 归属不悬空）

### 接口（`/api/users`，全部仅超级管理员）

```
GET    /users                              账号列表（含各自被授权的相册）
POST   /users                              新建账号（不传 password 则自动生成 12 位强密码；
                                           返回体里的 password 是明文，只返回这一次）
PUT    /users/{pid}/password               重置密码（同样只返回一次明文）
PUT    /users/{pid}/albums                 整体重设授权相册集合（前端勾选后保存）
POST   /users/{pid}/albums/{event_id}      单独授予某相册（相册详情页「授权已有账号」用）
DELETE /users/{pid}/albums/{event_id}      取消某相册授权（账号保留）
PUT    /users/{pid}/active                 启用 / 停用
PUT    /users/{pid}/role                   调整角色 super ↔ album
DELETE /users/{pid}                        删除账号（名下仍有相册/文件时拒绝）
```

`GET /api/auth/login` 与 `GET /api/auth/me` 现在都会返回 `role` 与 `is_super`，前端据此决定入口显隐。

### 使用方式（后台）

1. **一键为相册创建管理员**：进入相册详情 →「相册管理员」区块 → 输入用户名 → 点「为该相册创建管理员」→ 自动建账号（生成强密码，弹窗只显示一次，可一键复制）并自动授权本相册
2. **给已有账号授权/收回**：相册详情页「授权已有账号」下拉选择（或在该区块点 ✕ 收回）；也可在「用户管理」里点某账号的「授权相册」勾选后保存
3. **停用 / 删除 / 重置密码**：后台「用户管理」对应按钮；停用后该账号已登录的会话立即失效
